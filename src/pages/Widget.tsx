       ...
       <div className="flex flex-col w-screen h-screen scroll-smooth relative bg-slate-100">
            ...
            <div className="col-span-6 items-center px-24 justify-center w-full h-full mt-24 overflow-y-scroll">
                       ...
                       <div className="w-full flex flex-row content-between justify-between mt-16">
                           <DescriptionCard title={"Atendimentos"} value={`${res.attendances ?? 0}`} style={"bg-orange-600"} icon={ <MdAssignment size={120} color='white' opacity={.25} />} description={`Última atualização em: ${dataFormatada}`}/>
                           <DescriptionCard title={"Veículos"} value={`${res.vehicles ?? 0}`} style={"bg-green-600"} icon={ <GiTowTruck size={120} color='white' opacity={.25} />} description={"Todos os veículos estão em funcionamento"}/>
                           <DescriptionCard title={"Profissionais"} value={`${res.professionals ?? 0}`} style={"bg-blue-900"} icon={<MdOutlinePeopleOutline size={120} color='white' opacity={.25}/>} description={"Todos os profissionais estão em atuação"}/>
                       </div>
